...
class FiDBart(BartForConditionalGeneration):
    ...
    def wrap_encoder(self, use_checkpoint=False):
        ...
        Wrap T5 encoder to obtain a Fusion-in-Decoder model.
        """
        self.model.encoder = EncoderWrapper(self.get_encoder(), use_checkpoint=use_checkpoint)
    ...
    def get_crossattention_scores(self, context_mask):
        ...
        scores = []
        n_passages = context_mask.size(1)
        for mod in self.model.decoder.block:
            scores.append(mod.layer[1].EncDecAttention.score_storage)
        scores = torch.cat(scores, dim=2)
        bsz, n_heads, n_layers, _ = scores.size()
        # batch_size, n_head, n_layers, n_passages, text_maxlength
...
def apply_checkpoint_wrapper(BartEncoder, use_checkpoint):
    """
    Wrap each block of the encoder to enable checkpointing.
    """
    layers = []
    for mod in BartEncoder.layers:
        wrapped_mod = CheckpointWrapper(mod, use_checkpoint)
        layers.append(wrapped_mod)
    layers = nn.ModuleList(layers)
    BartEncoder.layers = layers
